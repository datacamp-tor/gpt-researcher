...
class DetailedReport:
    ...
    async def run(self, is_chinese: bool) -> str:
        ...
        report = await self._construct_detailed_report(report_introduction, report_body, is_chinese)
        return report
    ...
    async def _construct_detailed_report(self, introduction: str, report_body: str, is_chinese: bool) -> str:
        # 只传一个参数，接收两个返回值
        toc, detected_is_chinese = self.gpt_researcher.table_of_contents(report_body)

        # 你可以根据需要决定是否用 detected_is_chinese 覆盖传入的 is_chinese
        is_chinese = detected_is_chinese
        ...
        conclusion_with_references = self.gpt_researcher.add_references(
            conclusion, self.gpt_researcher.visited_urls, is_chinese
        )
