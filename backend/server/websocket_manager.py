...
async def run_agent(task, report_type, report_source, source_urls, document_urls, tone: Tone, websocket, stream_output=stream_output, headers=None, query_domains=[], config_path="", return_researcher=False, mcp_enabled=False, mcp_strategy="fast", mcp_configs=[], language: str | None = None):
    """Run the agent."""    
    ...
    if report_type == "multi_agents":
        ...
        if language and isinstance(language, str):
            is_chinese_flag = language.lower() in ["chinese", "zh", "zh-cn", "zh-hans"]

        report = await researcher.run(is_chinese=is_chinese_flag)
